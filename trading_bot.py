...
class TradingBot:
    ...
    async def buy_position(self, session, symbol, usd_amount, current_price, is_settled=True):
        ...
        # 1. Run Pre-Trade Simulation via review_equity_order
        review_args = {
        ...
        try:
            ...
            if not self.dry_run:
                logger.error("Aborting order placement due to simulation failure.")
                return

        if self.dry_run:
            logger.info("[DRY RUN] Pre-trade simulation complete. Order not sent to live market.")
            self.log_transaction({
                ...
                "notes": "Simulated buy order in dry-run mode via review_equity_order"
            })
            return
    ...
    async def sell_position(self, session, symbol, quantity, current_price, reason, avg_cost):
        ...
        if self.dry_run:
            ...
            self.log_transaction({
                ...
                "price": current_price,
                "reason": reason,
                "notes": "Simulated sell order in dry-run mode via review_equity_order"
            })
            return
